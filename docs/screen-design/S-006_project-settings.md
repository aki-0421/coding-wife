...
利用者が、現在選択中のプロジェクトまたはworkspaceだけへ適用されるContext、companion、履歴を、アプリ全体の設定と混同せず確認・変更・復旧できるようにする。
...
| キャンセル     | picker、preview、confirm開始前の保存済み値とdraftを維持する          |
| 閉じる操作     | Chat/Commit/Context tabまたは別workspaceを選択する                   |
| 再表示         | workspaceごとの保存済み値と同一WebView内のsection選択を復元する      |
...
| section navigation | Project context、Character context、Companion、History & privacy | section選択                       |
| settings main      | project identity、選択sectionのform/status/error                 | edit、save、import、retry、delete |
...
| 権限不足   | root/library/history scope拒否  | localized reason                                  | scope外操作不可            | repairまたは権限回復    |
...
Project / Character contextのfield、境界、conflict契約は[workspace sessions要件](../requirements/workspace-sessions.md)を正本とする。character importとmappingは[live2d companion要件](../requirements/live2d-companion.md)、history deleteは[activity history要件](../requirements/activity-history.md)を正本とし、S-006は選択scopeと画面遷移を追加で保証する。
...
| `WORK-F-048`, `WORK-F-057`, `WORK-F-063`, `WORK-F-066`                                    | workspace切替、Project/Character context   | [workspace-sessions](../requirements/workspace-sessions.md) |
| `LIVE-F-060`〜`LIVE-F-078`                                                                | project model、import、mapping、runtime    | [live2d-companion](../requirements/live2d-companion.md)     |
...
- [x] project-scoped 4 sectionとapp-global非対象が一意である。
