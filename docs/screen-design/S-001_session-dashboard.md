...
| active turn中のworkspace切替 | active/pending turnを持つold workspaceから別workspaceを選択 | old turnをinterruptせずnew workspaceを即時選択し、固有draft/summary/anchor/timelineを復元する。old workspace rowの実行表示を維持し、Composerにbackground executionを表示する。terminal後はlatest selectionのCodex contextを同一App Server process上でactivateする | 非該当 | selection永続化失敗時だけ元workspaceへ戻す。Codex activation失敗はselectionを戻さず再接続導線を表示する | `WORK-F-058`, `WORK-F-059`, `CODE-F-051`, `CODE-F-076` |
