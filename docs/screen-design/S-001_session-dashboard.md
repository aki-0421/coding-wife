...
| overview setup | native workspace読込完了後にCodex CLIまたはGit executableが未準備、registered projectが0件、またはOS/history/Live2D/preferencesに`blocked` / `unavailable`がある | sidebar、header、characterを表示しない全viewportの一面に、画面title、未完了件数、未完了項目だけを縦に表示し、title直下の導入説明は置かない。Codexはcopy可能な公式command、自動検出で見つからない場合の任意absolute path入力、自動検出へ戻す操作、再確認を、Gitは設定理由、公式command、再確認を、最初のprojectはnative folder pickerをprimary actionとして表示する。ready項目と内部診断のwarningは表示しない | command copy、検証済みCodex path保存・自動検出復帰、Project追加、native Project setup dialog、再確認、Quit。利用者のdefault shellを探索用にbounded実行する以外の任意shell実行とworkspace操作は不可 | 必須CLIがready、重大な内部診断が解消、registered projectが1件以上になった時。解消した行はsnapshotまたはproject state更新と同時に消す |
