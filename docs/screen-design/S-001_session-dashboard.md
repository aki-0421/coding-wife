...
| overview setup | native workspace読込完了後にCodex CLIまたはGit executableが未準備、registered projectが0件、OS/history/Live2D/preferencesに`blocked` / `unavailable`がある、選択workspaceのCodex activationが未接続・blocked・failed、またはnative runtimeがerror | sidebar、header、Chat、character、app logoを表示しない全viewportの一面に、画面title、未完了件数、未完了項目だけを縦に表示し、title直下の導入説明は置かない。未完了一覧headerには全項目を再診断する`再確認 / Recheck`を1個だけ置き、各項目には再確認buttonを置かない。Codexはcopy可能な公式command、自動検出で見つからない場合の任意absolute path入力、自動検出へ戻す操作を、Gitは設定理由と公式commandを、最初のprojectはnative folder pickerをprimary actionとして表示する。ready項目と内部診断のwarningは表示しない | 全体再確認、選択workspaceのCodex再activation、command copy、検証済みCodex path保存・自動検出復帰、Project追加、native Project setup dialog、Quit。利用者のdefault shellを探索用にbounded実行する以外の任意shell実行とworkspace操作は不可 | 必須CLIと選択workspaceのCodex接続がready、native runtimeと重大な内部診断が解消、registered projectが1件以上になった時。解消した行はsnapshot、workspace接続、runtime、project state更新と同時に消す |
...
| preflight再診断 | native readiness serviceが利用可能 | 同じsnapshotでGit/Codex/login/Sol/characterと内部診断を更新し、必要なら選択workspaceのCodex activationとnative runtimeも再試行する。overviewではreadyになった項目を除去する | 非該当 | 直前snapshotを保持して対象項目に再試行を示す | `WORK-F-048`, `CODE-F-051`, `CODE-F-075` |
...
初回overviewの判定正本は`NativeReadinessProvider`が保持する一つの`NativeReadinessSnapshotV1`、workspace adapterが復元したregistered project件数、選択workspaceのCodex接続状態、native runtime状態である。`src/features/readiness/SetupOverview.tsx`はCodex check、Git check内の`git_executable` fact、重大なinternal check、project件数と、`WorkspaceShell.tsx`が渡すCodex/runtime未接続overrideを未完了項目へ射影する。Git check全体の`warning`はactive repository未選択でも発生するため、overviewでGit導入済みかを判定する時はcheck statusではなく`git_executable=available`を使う。`WorkspaceShell.tsx`は`hydrationMode=native`かつsnapshot sourceが`native`の時だけこの射影でsidebar、header、各tab、Settingsを含む通常shell全体を置き換え、demo snapshotをnative readyとして扱わない。全体再確認ではnative readinessに加え、必要な選択workspaceのCodex activationとnative runtime refreshを再試行する。アプリ内ではCodex、Git、履歴の接続状態をまとめた個別の汎用noticeを描画しない。判定、日英copy、ready項目の非表示、最初のproject登録からworkspace project選択への復帰を変更した場合は`pnpm exec vitest run src/features/readiness/SetupOverview.test.tsx src/features/workspace-view/WorkspaceShell.test.tsx --fileParallelism=false`で同じ境界を確認する。
