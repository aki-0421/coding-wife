...
| オフライン | network/Codex接続なし | local list、Git/DB status、Codex offline、Composerに安全な`CODEX-...` reason codeと`再接続 / Reconnect` | project filter、Context、local project操作、選択workspaceの再接続が可能。Send不可。再接続中と失敗時もdraft不変 | 明示preflight成功 |
...
| preflight再診断 | native readiness serviceが利用可能 | setup snapshotではGit、Codex executable/App Server initializeと内部診断だけを更新し、readyになった項目を除去する。選択workspace再接続は非同期に開始し、schema/login/Sol/effortは通常workspaceの送信前診断で更新する | 非該当 | 直前snapshotを保持して対象項目に再試行を示す | `WORK-F-048`, `CODE-F-051`, `CODE-F-075` |
...
| active turn中のworkspace切替 | active/pending turnを持つold workspaceから別workspaceを選択 | old turnをinterruptせずnew workspaceを即時選択し、固有draft/summary/anchor/timelineを復元する。old workspace rowの実行表示を維持し、Composerにbackground executionを表示する。terminal後はlatest selectionのCodex contextを同一App Server process上でactivateする | 非該当 | selection永続化失敗時だけ元workspaceへ戻す。thread activation失敗はselectionとapp-wide接続表示を維持し、対象workspaceへthread再試行だけを表示する | `WORK-F-058`, `WORK-F-059`, `CODE-F-051`, `CODE-F-076` |
...
初回overviewの判定正本は`NativeReadinessProvider`が保持する一つの`NativeReadinessSnapshotV1`、workspace adapterが復元したregistered project件数、native runtime状態である。`src/features/readiness/SetupOverview.tsx`はCodex setup check、Git check内の`git_executable` fact、重大なinternal check、project件数と、`WorkspaceShell.tsx`が渡すruntime未接続overrideを未完了項目へ射影する。Git check全体の`warning`はactive repository未選択でも発生するため、overviewでGit導入済みかを判定する時はcheck statusではなく`git_executable=available`を使う。`WorkspaceShell.tsx`は`hydrationMode=native`かつreadiness snapshotが未確定の間は可視UIを描画せず、snapshot sourceが`native`で未解決項目が判明した時だけsidebar、header、各tab、Settingsを含む通常shell全体を置き換える。選択workspaceのCodex接続失敗だけではoverviewを表示せず、通常shell内のSend可否へ反映する。全体再確認またはpath設定成功後は必要な選択workspaceのCodex activationを非同期に開始し、native runtime refreshも再試行する。アプリ内ではCodex、Git、履歴の接続状態をまとめた個別の汎用noticeを描画しない。判定、日英copy、ready項目の非表示、最初のproject登録からworkspace project選択への復帰を変更した場合は`pnpm exec vitest run src/features/readiness/SetupOverview.test.tsx src/features/workspace-view/WorkspaceShell.test.tsx --fileParallelism=false`で同じ境界を確認する。
...
| Codex setup / workspace preflight | Rust supervisor | `run_diagnostic_check` / workspace Codex commands | setupはexecutable trust、version、App Server spawn、stable initializeだけ。workspace接続はschema、capability、auth、config、model/listを追加確認しauth内容はWebViewへ返さない | 前回結果維持 | setup failureはoverview、workspace failureは通常shellでSend不可のreasonを表示 |
