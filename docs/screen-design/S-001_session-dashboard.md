...
| Initial setup / Preflight | 初回project登録、Git、Codex executableとApp Server初期化のsetup readiness、通常workspaceでのlogin、GPT-5.6 Sol、character packの送信前診断と回復操作 |
...
| Codex supervisor | setup readinessとworkspace preflight statusだけ | setupではexecutable/App Server初期化、workspaceではschema/login/model capabilityを診断 | setup失敗ならoverview、workspace preflight失敗なら通常shellを維持してSend不可のstatusを返しthreadを開始しない |
...
| overview setup | native workspace読込完了後のsetup readinessでCodex CLIを検出できない、App Serverのstable initializeに失敗、Git executableが未準備、registered projectが0件、OS/history/Live2D/preferencesに`blocked` / `unavailable`、またはnative runtimeがerror | setup readinessが未完了の間は背景以外を描画しない。問題判明後だけsidebar、header、Chat、character、app logoを表示しない全viewportの一面に、画面title、未完了件数、未完了項目だけを縦に表示し、title直下の導入説明は置かない。未完了一覧headerには全項目を再診断する`再確認 / Recheck`を1個だけ置き、各項目には再確認buttonを置かない。未完了項目は利用者が報告できる安全な`READINESS-...` codeを本文に表示する。Codexはcopy可能な公式install command、自動検出で見つからない場合の任意absolute path入力、自動検出へ戻す操作を、Gitは設定理由と公式commandを、最初のprojectはnative folder pickerをprimary actionとして表示する。ready項目と内部診断のwarningは表示しない。workspaceのschema、login、model、effort失敗はこの状態へ進入させない | 全体再確認、command copy、検証済みCodex path保存・自動検出復帰、Project追加、native Project setup dialog、Quit。overviewでCodex path保存または自動検出復帰に成功した場合は選択workspace threadの開始または再開を自動実行するが、その完了までfieldまたはoverviewをprocessingに保たない。利用者のdefault shellを探索用にbounded実行する以外の任意shell実行とworkspace操作は不可 | Codex executableとApp Server初期化を含む必須setup check、native runtime、重大な内部診断が解消し、registered projectが1件以上になった時。解消した行はsnapshot、runtime、project state更新と同時に消す |
| 通常 | 1件以上のvalid workspace | group list。各group内はworkspace作成日時の新しい順、同時刻はWorkspace IDの昇順で固定する。active project、preflight、primary action 1件 | project filter、select、add、create、state action | 操作開始、error |
...
| App Server復旧中 | App Server crash、EOF、protocol違反または一時的なhandshake失敗 | 通常workspaceとdraft/timelineを維持し、接続状態、reason code、badge/banner、Reconnect/Retryを表示しない | project filter、Context、local project操作は可能。Send不可。App Serverと選択workspace threadは自動復旧する | handshakeと選択workspace thread activation成功 |
...
| background execution | active/pending turnを持つworkspaceから別workspaceを選択 | new workspaceを即時選択し、その固有draft、summary、anchor、timelineを表示する。old workspace rowは実行中表示を維持し、Composerはold workspace名とturn継続中であることを表示する | local閲覧、Context編集、workspace選択は可能。新しいSend、attachment追加、old turnのStop/decision/approvalは不可 | old turnがterminalになりlatest selectionのCodex contextがactiveになる |
...
| preflight再診断 | native readiness serviceが利用可能 | setup snapshotではGit、Codex executable/App Server initializeと内部診断だけを更新し、readyになった項目を除去する。選択workspace thread activationは非同期に開始し、schema/login/Sol/effortは通常workspaceの送信前診断で更新する | 非該当 | 直前snapshotを保持して対象項目に再試行を示す | `WORK-F-048`, `CODE-F-051`, `CODE-F-075` |
...
| workspace選択 | itemがMissing以外 | header、Chat、Commit、Characterを同一IDへ100ms以内にatomic切替。選択時に更新される`last_selected_at`、`updated_at`ではsidebarの作成日時順を変更しない。App SettingsのProject detail / Character detailはworkspace tab状態へ含めない | 非該当 | 永続化失敗時は元workspaceへ戻し、safe error codeを表示 | `WORK-F-052`, `WORK-F-054`, `WORK-F-059` |
| active turn中のworkspace切替 | active/pending turnを持つold workspaceから別workspaceを選択 | old turnをinterruptせずnew workspaceを即時選択し、固有draft/summary/anchor/timelineを復元する。old workspace rowの実行表示を維持し、Composerにbackground executionを表示する。terminal後はlatest selectionのCodex contextを同一App Server process上でactivateする | 非該当 | selection永続化失敗時だけ元workspaceへ戻す。thread activationの一時失敗はselectionとdraftを維持して自動再試行し、接続状態や手動Retryを表示しない | `WORK-F-058`, `WORK-F-059`, `CODE-F-051`, `CODE-F-076` |
...
初回overviewの判定正本は`NativeReadinessProvider`が保持する一つの`NativeReadinessSnapshotV1`、workspace adapterが復元したregistered project件数、native runtime状態である。`src/features/readiness/SetupOverview.tsx`はCodex setup check、Git check内の`git_executable` fact、重大なinternal check、project件数と、`WorkspaceShell.tsx`が渡すruntime未接続overrideを未完了項目へ射影する。Git check全体の`warning`はactive repository未選択でも発生するため、overviewでGit導入済みかを判定する時はcheck statusではなく`git_executable=available`を使う。`WorkspaceShell.tsx`は`hydrationMode=native`かつreadiness snapshotが未確定の間は可視UIを描画せず、snapshot sourceが`native`で未解決項目が判明した時だけsidebar、header、各tab、Settingsを含む通常shell全体を置き換える。初回setup完了後のApp Server接続失敗だけではoverviewを表示せず、通常shell内のSend可否へ内部反映して自動復旧する。全体再確認またはpath設定成功後は必要な選択workspaceのCodex activationを非同期に開始し、native runtime refreshも再試行する。アプリ内ではApp Server接続状態、reason code、Codex/Git/履歴の接続状態をまとめた汎用notice、手動Reconnect/Retryを描画しない。判定、日英copy、ready項目の非表示、最初のproject登録からworkspace project選択への復帰を変更した場合は`pnpm exec vitest run src/features/readiness/SetupOverview.test.tsx src/features/workspace-view/WorkspaceShell.test.tsx --fileParallelism=false`で同じ境界を確認する。
...
| Codex setup / workspace preflight | Rust supervisor | `run_diagnostic_check` / workspace Codex commands | setupはexecutable trust、version、App Server spawn、stable initializeだけ。app-wide handshakeはschema、capability、auth、config、model/listを追加確認しauth内容はWebViewへ返さない | 前回結果維持 | setup failureはoverview、初回setup後の接続failureは通常shellでSendだけを無効にして自動復旧し、reasonは表示しない |
...
| workspace Archive | App-owned Codex controller → Rust Git process + DB | active/pending時はexact generationの`turn/interrupt`とterminal cleanup、続いて`workspace_archive` | typed workspace ID、active時のexpected generationとterminal history flush、app-owned root containment、固定`git worktree remove --force`。idleはinterruptなし、missing targetは成功扱い | active時の確認前または`戻る`では変更なし | stale/interrupt/cleanup/flush失敗では`workspace_archive`を呼ばない。削除失敗でも他worktree/project/refを変更しない |
...
| `CODE-F-051`, `CODE-F-075` | Codex setup probeとworkspace login/Sol preflightの分離、blocked reason | [codex-main-session](../requirements/codex-main-session.md) |
