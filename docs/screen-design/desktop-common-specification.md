...
|---:|---|---|---|
| 1470px以上 | sidebar 255.04px固定。超過幅はmainへ与える | Chat/ContextとCompanionを1:1で拡張。Commitはevidenceを広くしてCompanionを縮小 | tab row固定、長値ellipsis |
| 1280〜1469px | sidebar 255.04px固定 | 残幅をprimary work surface/Companionへ配分。primaryが500px未満になる前にCompanionを縮める | composerはChat内で左右18px |
| 960〜1279px | 64px icon rail。workspace listはbuttonからportal drawer | primaryを最低520px、Companionへ残幅。decision/review時はCompanionをcompact化 | tabsはhorizontal scroll、footer controlsはwrap |
| 200% text zoom | 64px rail + drawerを使用 | Companionをhide可能、Chat/decisionを優先 | labelを縮小せずwrap/overflow menu |

選択workspaceのChat、Commit、Contextは同じCompanion instanceを右paneへ継続表示し、tab切替でcanvasを再生成しない。S-003はevidenceを優先してCompanionを縮小し、commit listを非modal drawerへ移せる。S-005とS-006はCompanionを表示せずsetting formを全幅で構成する。character visibilityがHiddenの場合もprimary work surfaceを全幅へ戻し、visible HTML stateを残す。Chat、Commit、ContextとCompanionの間へdividerまたは別cardを置かない。
