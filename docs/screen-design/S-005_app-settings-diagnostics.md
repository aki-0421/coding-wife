...
description: "登録projectごとのProject contextと、全projectへ共通適用するcharacter presentation、表示、音声、診断を管理する画面仕様。"
updated: 2026-07-20
read_when:
...
Project ID単位のProject context、AppPreferences、app-global Character context、character library selection/mapping、Narration settings、readiness snapshotの正本と失敗契約は各要件定義書に従う。コミット説明の内部実行単位、model policy、利用量、監査metadataはS-005に表示しない。Projects一覧と詳細の表示だけではworkspace history、Git state、他projectのdraftを変更しない。
...
- [x] app settingsの6 section、Projects内のproject-scoped詳細、workspace-scoped非対象が一意である。
