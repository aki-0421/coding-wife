...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Global enable        | fresh/missing `SupportSettingsV1`の希望値は既存挙動を維持するためon。実効onはapproved CLI version・binary hash・schema fingerprintとisolation capability合格時だけ。専用clean runtime、wire-advertised/external-authority tool 0件、`tools` field不在、空のowner-only cwd、runtime root 0件をread-only表示 | off更新は保存とenqueue gateを直列化し、queued全件とactive commit-support jobを停止してから完了する。off、capacity 0、capability不足、tool-absence boundary不一致、または非承認binaryではsupport process/model invocationを作らずdeterministic fallbackを維持する |
| Commit explainer     | fresh/missing recordの希望値on。global希望値とは独立してnativeへ永続化し、再起動後に復元する                                                                                                                        | verified new commitは`auto_verified_commit`で自動job、未生成の既存commitは`user_request`、failed/canceledは`user_retry`で起動し、redacted `CommitEvidenceV1`最大64KiBだけを読む。off更新は該当queued/active jobを停止する |
...
| Usage / audit        | active/queued/max capacity、attempted/started/succeeded/failed/canceled/unavailable、input/output/total token、total latency、latest outcome、last error、固定model/effort/permission/skill/non-persistence | prompt/response本文、private path、credentialを表示・保存しない                                                                                                                 |

Support sectionは実装済みのglobalとcommit explainerだけを表示し、未実装のpresence/narrationとdecision explainerをdisabled controlやcoming-soon rowとしても表示しない。toggle保存中は二重操作を無効にし、保存・停止失敗をcontrol直下へsafe code付きで表示する。role toggleをoffにするとqueued taskをcancelし、新規invocationを作らない。active taskは5秒以内にCanceled/Timeoutへ遷移させ、main turnを継続する。non-persistence release auditの最終resultと実施日時へDiagnosticsから到達できる。
