...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Global enable        | release proof済みcapacity 1の時だけdefault on。専用clean runtime、external-authority tool 0件、wire exact inert `update_plan` 1件、空のowner-only cwd、runtime root 0件をread-only表示 | capacity 0、capability不足、tool schema/hash不一致、またはoffではthreadを起動せず、queued/activeをcancelしてdeterministic fallbackを維持。`update_plan`実callも当該taskをfailedにする |
...
| Support       | enable、capacity 1、external-authority tool 0件、inert tool名/schema/hash、permission profile、clean runtime/auth bridge/canary、`coding-wife-explain-commit` version/digest、last usage | capacity 0、isolation/auth/tool mismatch、inert tool実call、timeout、output schema、policy、non-persistence未検証 | Cancel、Disable、Recheck                            |
