...
旧workspace側editor entryと混在Settings間のcross-navigation契約は廃止した。現行UIではProject ContextをS-005のProjects detail、Character ContextをS-005のCharacter contextだけで編集し、workspace Settings tab自体を置かない。以下の状態表は旧画面の履歴であり、現行実装の正本にはしない。
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Global enable        | freshかつ安全に新規作成した`SupportSettingsV1`の希望値だけ既存挙動を維持するためon。既存directory/fileのowner、type、group/other permissionをchmod前に検証し、unsafe/corrupt storeはoff/off。実効onはapproved CLI version・full binary SHA-256・full schema SHA-256・full skill digest/versionとisolation capability合格時だけ。prefixは表示専用 | off更新は保存とenqueue gateを直列化し、queued全件とactive commit-support jobを停止してから完了する。cleanup不収束時もUIは再取得した保存済みoff/versionとsafe errorを表示する。off、capacity 0、capability不足、tool-absence boundary不一致、またはfull identity不一致ではexecutor/model invocationを作らずdeterministic fallbackを維持する |
