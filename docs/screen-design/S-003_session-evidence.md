...
| Explanation | 明示trigger、background status、Cancel、streamed caption、deterministic fallback |
...
| アクター | 入力 | 操作 | 権限境界 |
|---|---|---|---|
| 利用者 | selection、filter、explicit explanation request | inspect、Refresh、詳しく教えて、Cancel、Chatへ戻る | Git mutationなし |
...
| Actions | `詳しく教えて`、説明中の`Cancel`。Commit/Restore/Branchは存在しない |

「詳しく教えて」はactive selection、Freshなworkspace generation、valid redactionの時だけenabledにする。disabled時は理由をbutton近傍のtextで示す。
...
button押下時に`CommitExplanationRequestedV1`を発行し、次を満たす`CommitEvidenceV1`だけを渡す。
...
isolated support turnには`coding-wife-explain-commit`をexplicit skill inputで1件注入する。deltaはrequest ID、source commit ID、generation、locale、sequence、text、doneを検証後、characterのvisible HTML captionへ逐次適用する。
...
| 通常 | 1件以上 | list、selection、detail、gate | inspect、Refresh、詳しく教えて | selection/refresh/explanation |
...
| Explanation queued/running | 明示request後 | button progress、caption portal、Cancel | Cancel、read-only inspect | completed/canceled/unavailable |
| Explanation completed | done受理 | caption terminal、Replayなし | 新規request、inspect | selection/new request |
| Explanation canceled | user/route/workspace stop | canceled caption | 新規request | queued |
| Explanation unavailable | support off/offline/schema/redaction error | deterministic explanation unavailable text | inspect、Settings、Retry | queued |
...
| 詳しく教えて | active selection、redaction pass | background explanation開始 | Cancelでterminal化 | unavailable caption | `GIT-F-090`〜`GIT-F-095` |
...
|---|---|---|---|
| `commit_explanation_requested` | EvidenceView | support runtime | schema version、request ID、workspace generation、commit evidence ID、locale |
| `commit_explanation_started` | support runtime | EvidenceView/caption | request ID、skill audit、startedAt |
| `commit_explanation_delta` | support runtime | caption/TTS policy | request ID、commit ID、generation、locale、sequence、text、done |
| `commit_explanation_terminal` | support runtime | EvidenceView/HIST metadata | request ID、status、usage、latency、error code |
| `commit_explanation_cancel_requested` | EvidenceView | support runtime | request ID、generation、reason |
...
- focus順はobserver bar、commit list、detail header、tabs、file list、diff、詳しく教えて、caption Cancelとする。
...
| `GIT-F-090`〜`GIT-F-095` | 詳しく教えて、redaction、caption、TTS、fail closed | [Git observer要件](../requirements/git-review-harness.md) |
| `SUP-F-069`〜`SUP-F-077` | isolated commit explainerとstream schema | [Support要件](../requirements/support-agent-orchestration.md) |
