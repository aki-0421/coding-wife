...
description: "main Codexが作ったcommitをGitHub Files changed型のcompactなlist、file tree、unified diffと安全な説明導線でread-only表示する。"
updated: 2026-07-22
read_when:
...
利用者が、main Codexの作業をGitHubのcommit changesと同じ読み順で、commit identity、変更量、file tree、sanitized unified diffから確認できるようにする。監査用metadata、verification、decision、riskはnative/HISTで収集・保持するが通常UIには投影しない。commit producerは`coding-wife-commit-work`を毎turn受け取るmain Codexであり、本画面とnative Git backendはGit状態を変更しない。
...
| 対象 | 内容 |
|---|---|
| Commit list | compact drawer内のsubject、short SHA、author、relative authored time |
| Commit changes | subject、author、relative authored time、short SHA、change stats、file navigator、1 fileずつのlazy unified diff |
...
| 正常終了 | commit/file selectionとscrollを保存してChatへ戻る。Git状態は不変 |
...
| アクター | 入力 | 操作 | 権限境界 |
|---|---|---|---|
| 利用者 | commit/file selection、local path filter、app controller state | inspect、Refresh、presentation表示/Close、詳しく教えて、retry、生成Cancel、Chatへ戻る | Git mutationなし。説明intentをmain sessionへ送らない |
...
1. selected commitのsubject、author、relative authored time、short SHA、change stats。
2. changed file pathとfile単位のaddition/deletion。
3. old/new line number付きsanitized unified diff。
4. 利用者が明示した時だけのcommit explanation。

observer policy、correlation、verification、decision、risk、skill audit、internal IDは保存・診断対象であり、通常のreview surfaceには描画しない。失敗は安全な利用者向けtextで示し、native error codeやraw reasonを表示しない。

## レイアウト

sidebarと81px headerはS-002と同じ位置を維持し、Commit tabをactiveにする。main bodyはchanges primary surfaceとS-002から継続するCharacter paneで構成し、同じLive2D canvas instanceを表示する。CharacterはChatと同じ横幅を維持し、commit listはcompact triggerから開く非modal drawerに限定する。

| 領域 | 標準幅・高さ | 内容 | resize時 |
|---|---|---|---|
| compact toolbar | changes上40px | commit drawer trigger、icon-only local refresh | 1行を維持し内部propertyを追加しない |
| commit list | 非modal drawer、最大300px | subject、short SHA、author、relative authored time | trigger、Escape、outside clickで開閉しfocusを復元 |
| changes | primary surfaceの全幅 | selected commit header、file navigator、selected-file unified diff、単一explanation action | 760px以下はfile navigatorをcompact selector化 |
...
Characterは常時表示し、commit listはwindow幅に関係なく非modal drawerだけで表示する。caption portalはchangesを覆わないようCharacter内へ置き、canvasがtext-onlyへ縮退した場合もHTML captionを維持する。200% text zoomでroot横scrollを要求せず、diff bodyだけ内部scrollを許す。

## コンポーネント

### Compact toolbar

branch、HEAD、Fresh/Stale、last observation reason/time、pre-existing change、All/Work unit/Needs attention filter、read-only badgeを通常UIから完全に外す。commit drawer triggerとlocal observation refreshだけをicon主体で置き、tooltipとJA/ENのaccessible nameを与える。
...
1. subject（1〜2行）。
2. short SHA、author display name、relative authored timeを持つ1行。
...
| 項目 | 内容 |
|---|---|
| Identity | subject、author display name、relative authored timeを持つ`time[dateTime]`、short SHA、`N files changed +A −D` |
| Copy | SHAはicon-only actionでhover / focus時だけ表示し、tooltip、accessible name、polite resultを持つ |
| Hidden metadata | full SHA text、email、committed time、parents、work unit/source/before/after observation ID、producer、Persisted、objective/acceptance、caution/property cardを描画しない |
| Action | controller stateに応じた単一位置の`詳しく教えて` / Show / Cancel / Retry。Commit/Restore/Branchは存在しない |
...
desktopはlocal path filter付きfile navigatorとselected-file single-column diffを置く。file rowはopaque `fileEvidenceId`、表示用redacted relative path、added/deleted linesだけを描画し、Arrow Up/Downで前後fileへ移動できる。primary surfaceが760px以下または200% text zoom時はnavigatorを隠し、同じfile群のcompact selectorへ置き換える。WebViewはraw filesystem pathやGit argsをnativeへ返さない。

detail受理時に先頭fileを自動選択し、その1 fileだけを`read_commit_diff_file`でlazy loadする。selection変更時は旧responseを破棄し、全file diffの同時取得を禁止する。selected file headerはcollapse toggle、relative path、局所`+/-`、hover / focus時だけのicon-only copy pathを持ち、change kind/property badgeを置かない。

| state | 表示 |
|---|---|
| text | `@@ -old,+new @@`から採番したold/new line gutter、`+/-/ ` marker、context付きunified diff。`diff --git` / `index` / `---` / `+++` metadataは非表示 |
...
| empty | 変更行がないことだけを短いtextで表示 |
| error | 安全な短いtext。internal error codeを表示せず他fileは閲覧可能 |
...
| 通常 | 1件以上 | compact commit selector、selected identity、file navigator、1 fileのdiff、単一explanation action | inspect、Refresh、file/commit selection、state別action | selection/refresh/explanation |
...
| UI preference | app settings | commit/file selection change | 次回active表示 | local path filter、scroll中間delta |
...
- focus順はcompact toolbar、commit drawer、selected header、file filter/list、selected file header/diff、単一の`詳しく教えて` state action、caption Close / muteとする。
...
- diff kindはold/new line gutter、`+/-/ ` marker、screen reader label、背景を併用し、色だけで伝えない。
...
| `GIT-F-079`〜`GIT-F-083` | main skill auditの内部収集とproducer境界 | [Git observer要件](../requirements/git-review-harness.md) |
| `GIT-F-084`〜`GIT-F-089` | compact list、selected identity、file navigation、unified lazy diff、内部gate、performance | [Git observer要件](../requirements/git-review-harness.md) |
...
- [x] compact list、selected identity、file filter/navigation、unified lazy diff、internal property非表示、空・error・stale・oversizeを定義した。
