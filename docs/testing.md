...
Tauri produces only `Coding Wife.app`. The app command builds in a fresh private target, keeps raw tool output in a mode-`0700` work directory and mode-`0600` log, normalizes the candidate app root to distribution mode `0755`, and requires production Hiyori discovery to stay inside installed resources rather than embedding a source checkout fallback. It checks that locked dependency notices are current and packaged legal files are byte-identical, excludes the development-only demo runtime, signs nested executable code first, and seals the complete app with a timestamp-free ad-hoc signature. It full-verifies the private and same-filesystem ready copies before publishing the app and its run manifest. The combined command supplies a private output, so the child app build never changes the canonical path. Raw verifier output remains private; an app verification failure may expose only the verifier's allowlisted `APP_*` cause code. This integrity seal does not provide a Developer ID identity and does not prove notarization.
