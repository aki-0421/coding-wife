...
| evidence snapshot | sessionの特定時点について、HEAD、branch、Git state fingerprint、command、test、reviewへの参照を固定した構造化record。 |
| final evidence | 最新snapshotまでの目的、変更、検証、review、commit、残課題をS-003で再表示するread-onlyなまとめ。 |
| Git state fingerprint | HEADのsymbolic refとOID、indexのstage・mode・object・path hash、tracked worktree diff hash、untracked manifestのpath・種別・内容hash、merge・rebase等のoperation markerから算出する識別値。file内容そのものは保存しない。 |
...
| 競合と秘密漏えいを抑える | アプリ管理turnをworktree別leaseで直列化し、外部変更は安定snapshotで検出し、秘密値とsource本文を永続evidenceから除外できる。 |
...
| 競合・復旧 | support要件のcanonical worktree別turn lease、安定snapshot、stale判定を共有し、再起動後にevidence indexを復元する。 |
...
| main agent（Sol） | ユーザー指示と現在状態から作業を判断する主体 | Full accessのtool操作、skill選択、support依頼、結果説明 | turn leaseを取得できない、または固定session境界が不一致ならturnを開始しない |
...
| turn lease取得不能 | 任意 | Gitを読まず`read_only_evidence_unavailable`でincompleteにする |
| 収集中にGit state fingerprint変化 | 任意 | 結果を破棄して1回再収集し、再変化なら`UNSTABLE`でincompleteにする |
...
| 領域 | 要件 |
|---|---|
| セキュリティ | collectorはallowlist済みread-only Git commandを引数配列で実行し、WebView入力をshellへ渡さない。turn leaseはアプリ管理turnだけを直列化し、Full accessまたは外部processをsandbox化すると誤表示しない。 |
