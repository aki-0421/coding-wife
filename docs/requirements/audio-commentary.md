...
| `NARR-F-058` | transcriptはaudioより先に表示される         | narration event受理から300ms以内にactive viewportのHTML captionへtextをcommitし、caption componentがlayout後のpaint境界を通過してvisibleであることをackした時点から100ms以上表示した後にだけlocal TTS processを開始する。component未mount、hidden、ack timeoutでは発話せずcaption-onlyへterminal化する | Approved | 非該当           |
...
| `NARR-F-067` | local adapter失敗はtext fallbackになる | binary missing/tampered、voice unavailable、spawn/stdin/exit error、audio deviceなし、5秒test timeoutの各場合にaudio unavailableを即時terminal表示し、nativeのsanitized `lastErrorCode`、caption、main turnを維持する。native runtimeが`unavailable`になった時はfrontend timeoutまでpollせず、frontend watchdog時はnative cancelを完了してからterminal化する。offlineは失敗条件にしない | Approved | 非該当           |
...
| 要件ID       | 要件                                               | 受け入れ条件                                                                                                                                                                                                                                       | 状態     | 廃止理由・後継ID |
| ------------ | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------- |
| `NARR-F-072` | local processへredacted transcriptだけを渡す       | scalar count 1〜240、NULなしで、括弧・引用符・`=`・`:`直後を含むPOSIX absolute path、Bearer token、GitHub token、AWS access/secret/session key、Slack token、PEM private key、cookie/session/password/token/key assignmentを検出したtextをfrontend/nativeの同一fixture集合でrejectする。検査済みtextだけをstdinへ書き、command line、environment、file、networkへ本文を渡さない。引数は`-v <exact allowlist voice> -r <validated integer>`だけで、shell、`-f`、`-o`、`-n`、`-a`を使用しない | Approved | 非該当           |
...
| `NARR-F-081` | TTSはactive captionと同じchunkだけを読む                          | TTS enabledかつunmutedの時だけ、captionへ確定した同一commit key・同一text・同一sequenceをlocal adapterのstdinへ渡し、音声用の追加要約・言い換え・evidence再送を0件にする                                                                                             | Approved | 非該当           |
...
| WebView contract・controller | `src/features/narration/contracts.ts`、`controller.ts`、`transport.ts`                                    | `background_support`以外を受理せず、未activate jobはcaption/TTSへ出さず、dismissとsupport cancelを分離し、unsafe stream後のlate chunkを復活させない |
...
| Audio設定                    | `src/features/narration/components/NarrationSettings.tsx`、`src/features/workspace-view/SettingsView.tsx` | default off、明示Save、visible test caption、native設定を正本にし、voice retryとdirty draft保持を全error stateで検証する                 |
| character caption・mute      | `src/features/narration/components/CommitNarrationCaption.tsx`、`CharacterStageSlot.tsx`、`ChatView.tsx`  | accepted chunkを順番どおりvisible HTML/live regionへ出す。active viewport内の正の矩形かつ前面hit targetと確認できた時だけackし、100ms lead後に発話する。clipped/occluded captionはackせず、840px以下でもmobile captionを残す |

`pnpm exec vitest run src/features/narration/**/*.test.ts src/features/narration/**/*.test.tsx src/features/workspace-view/CharacterStageSlot.narration.test.tsx --testTimeout=20000 --fileParallelism=false`を実行すると、strict envelope、IPC payload、default off、明示presentation、active viewport paint ack + 100ms caption lead、same-text speech、mute、dismiss/replay、support cancel、native unavailable、stale generation、voice retry、dirty draft、ja/en status、responsive側のcomponent境界を検証できる。続けて`pnpm exec tsc --noEmit --pretty false`と対象pathのESLintを実行する。実ブラウザでは1470px、960px、480pxと200%相当幅で、captionの実幅、mobile fallback、ja/en、reduced motion、TTS off/on、mute、Close→reopen、Cancel、settings error→Retryを確認する。
