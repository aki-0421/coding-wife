...
| --------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| ローカル利用者                    | TTS設定と再生を管理                                                                             | provider credential、enable、model、voice、speed、Companion mute、test、disable、reset | provider/network/player失敗時も字幕を維持する                              |
...
| 要件ID       | 要件                                   | 受け入れ条件                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 状態     | 廃止理由・後継ID |
| ------------ | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------- |
| `NARR-F-064` | TTSは初回とreset後に無効である         | fresh profileとReset Audio Settings後にTTS toggleがoffで、provider未選択、API key未設定、network request、audio fileが0件になる                                                                                                                                                                                                                                                                                                                                                                                                          | Approved | 非該当           |
...
| `NARR-F-066` | 利用者はmodel、voice、speedをpreviewできる | provider、API key、allowlist model/voice選択後に固定sample captionをmountし、sample自身のvisible ackと100ms lead後だけrequest/replayする。speedは0.75〜1.25を0.05刻みとし、caption未mount/hidden/ack timeout、cancel、network/playback timeout時も設定入力を保持する                                                                                                                                                                                                                                                                      | Approved | 非該当           |
...
| `NARR-F-089` | Audio設定の失敗復旧で入力とlocaleを維持する                       | settings取得済み状態でprovider/voice取得、save、testが失敗してもinline safe error codeを表示し、保存済み設定と未保存API key/model/voice/speed draftを維持する。mute保存によるsettings version更新でdirty draftを再初期化しない。Audio設定にはmute fieldとactive commit presentation statusを表示しない                                                                                                                                     | Approved | 非該当           |
...
| 未保存データ                | API key/model/voice/speedの未保存入力はroute離脱で破棄                                  | `NARR-F-066`                             |
...
| Audio設定                    | `src/features/narration/components/NarrationSettings.tsx`、`src/features/workspace-view/SettingsView.tsx`                        | TTS toggleを先頭、設定済みprovider select、provider tabs、password API key、model/voice/speed、default off、明示Save、Test sample自身のvisible ack + 100ms lead、dirty draft保持を検証する。muteとactive commit presentationは表示しない                                                                                                                    |
