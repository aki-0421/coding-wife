...
| character caption・mute      | `src/features/narration/components/CommitNarrationCaption.tsx`、`CharacterStageSlot.tsx`、`ChatView.tsx`  | accepted chunkを順番どおりvisible HTML/live regionへ出し、paint ackと100ms lead後だけ発話し、840px以下でもmobile captionを残す             |

`pnpm exec vitest run src/features/narration/**/*.test.ts src/features/narration/**/*.test.tsx src/features/workspace-view/CharacterStageSlot.narration.test.tsx --testTimeout=20000 --fileParallelism=false`を実行すると、strict envelope、IPC payload、default off、明示presentation、paint ack + 100ms caption lead、same-text speech、mute、dismiss/replay、support cancel、native unavailable、stale generation、voice retry、dirty draft、ja/en status、responsive側のcomponent境界を検証できる。続けて`pnpm exec tsc --noEmit --pretty false`と対象pathのESLintを実行する。実ブラウザでは1470px、960px、480pxと200%相当幅で、captionの実幅、mobile fallback、ja/en、reduced motion、TTS off/on、mute、Close→reopen、Cancel、settings error→Retryを確認する。
