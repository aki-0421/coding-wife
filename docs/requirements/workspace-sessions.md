...
| `WORK-F-048` | 利用者は初回起動と送信前にpreflightを確認できる | native workspace読込後、Codex CLIまたはGit executableが利用できない時は通常shellより先に全viewportのoverview setup guideを表示する。guideはnative readiness snapshotから未解決項目だけを列挙し、ready項目を描画しない。Codexはinstall、login、update、GitはmacOSのinstallを現在のrecovery actionに合わせて日英で示し、commandは利用者が明示的にcopyして外部terminalで実行する。OS、history、Live2D、preferencesの`blocked`または`unavailable`も同じ画面に回復理由を示すが、`warning`だけではoverviewを継続しない。readiness再確認は項目ごとに置かず、未完了一覧全体に対するbuttonを1個だけ表示する。guideから任意shellを起動せず、snapshot再確認で解消した項目は直ちに非表示にする。workspaceではGit、Codex executable、auth、`gpt-5.6-sol`、character packを`ready/warning/blocked`で表示し、blocked項目があればSendを無効にする | Approved | 非該当 |
