...
| project root自体をworkspaceとして自動登録 | projectとworktreeの区別を保つ | 利用者がmain surfaceまたはsidebarのWorkspace追加でproject cardを明示選択し、app-owned worktreeを作成する |
...
| `WORK-F-048` | 利用者は初回起動と送信前にpreflightを確認できる | native workspace読込後、Codex CLIのtrusted executableを検出できない、Codex App Serverを起動して`initialize` / `initialized`を完了できない、Git executableが利用できない、native runtimeがerror、registered projectが0件、またはOS、history、Live2D、preferencesが`blocked` / `unavailable`の時だけ、通常shellより先に全viewportのoverview setup guideを表示する。起動時のnative readinessはworkspace復元完了barrierを待ち、復元対象がある場合は保存済みCodex pathを復元済みworkspace rootで検証する。Codex binaryのdiscovery、trust、version、App Server初期化を一つのaggregate timeoutで打ち切らず、各検証結果からsetup判定を確定する。同じsetup probe内で直前に完全検証したbinary identityは次のprocess境界まで再利用し、spawn直後の差し替え検知を維持したまま連続する重複full hashを行わない。workspace履歴のhydration中は空windowにせず既存のloading UIを表示し、履歴復元後はnative readinessまたはCodex sessionの完了を待たず通常shellを描画する。readiness未確定中はsetup guideを推測表示せず、問題が判明した後だけguideを描画する。guideはnative readiness snapshotから未解決項目だけを列挙し、ready項目を描画しない。Codexのsetup判定ではbinary trust、version、App Server起動と初期化だけを確認し、schema、login、config、model/list、`gpt-5.6-sol`、reasoning effortを待たない。これらの送信前診断は通常workspaceでbackground継続し、初回setup後のApp Server接続中または一時失敗時はtimeline、draft、navigationを維持してSendだけを無効にし、失敗だけではsetup guideへ戻さない。接続状態、接続error code、接続badge/banner、手動Reconnect/Retryは通常workspaceへ表示せず、App Serverと選択workspace threadを自動復旧する。CodexとGitのinstall commandは利用者が明示的にcopyして外部terminalで実行する。readiness再確認は項目ごとに置かず、未完了一覧全体に対するbuttonを1個だけ表示する。overviewで明示Codex pathのbinary trust、version、App Server初期化と保存に成功した時はguideを直ちに終了し、選択workspace threadの開始または再開は追加clickなしで開始するが、その完了をpath fieldまたはguideの待機条件にしない。guideから任意shellを起動せず、snapshot更新で解消した項目は直ちに非表示にする。アプリ内の各画面にはCodex、Git、履歴の接続状態をまとめた個別の汎用noticeを表示しない。workspaceではGit、Codex executable、auth、`gpt-5.6-sol`、character packを`ready/warning/blocked`で表示し、blocked項目があればSendを無効にする | Approved | 非該当 |
...
| 要件ID | 要件 | 受け入れ条件 | 状態 | 廃止理由・後継ID |
|---|---|---|---|---|
| `WORK-F-050` | 利用者は登録projectにworkspaceを作成できる | workspace 0件のmain surfaceとsidebarの`+`から開くproject選択dialogは、同じproject選択contentを使う。登録projectをrepository icon画像とGitHub `owner/repo`、取得できない時はproject名を持つcard風buttonとして3columnで列挙し、table、select、workspace name input、補助説明を表示しない。cardを選ぶと対象Project IDと`ws-MMDD-<random 4文字>`形式の既定nameで作成を開始する。projectの現在HEADを起点にapp dataのexact `worktrees/<Project ID>/<Workspace ID>`配下へ新branchとGit worktreeを作り、workspace固有rootとper-worktree Git directoryが登録projectと同じGit common directory identityであることを照合する。workspace rowの作成と同じtransactionでworkspace root、`.git` marker、per-worktree Git directory、common Git directoryのcanonical pathとfilesystem identityをversioned managed provenanceとして保存し、全項目成功後だけBacklogへ追加・選択してそのworkspaceを開く。dialog経由ならdialogも閉じる。処理中の重複操作と失敗時のselection変更を行わず、失敗時は同じproject cardから再試行できる。同じprojectに複数worktreeを持てる | Approved | 非該当 |
...
| `WORK-F-052` | 利用者はlifecycle groupからworkspaceを選択できる | app localeにかかわらずLinearと同じ英語のDone/In Review/In Progress/Backlog/Canceledでgroupを表示し、各statusをcheck、half-filled progress、quarter-filled progress、dotted、xの円形iconで識別できる。同一group内はworkspace作成日時の新しい順、同時刻はWorkspace IDの昇順で固定し、選択、履歴更新、draft保存では並びを変更しない。各groupは初期展開され、heading行全体のclick、`Enter`、`Space`で他groupとselectionを変えず独立して開閉できる。toggleは`aria-expanded`と`aria-controls`を持ち、chevronはhoverまたはfocus-visible時だけ表示する。折り畳み中だけ対象workspaceの数値件数を0件を含めて表示し、accessible nameでは展開状態にかかわらず件数を1回だけ伝える。item選択でheader、Chat、Commit、Characterが同一workspaceへ100ms以内に切り替わる | Approved | 非該当 |
...
| `WORK-F-054` | アプリは現在のrepoとbranchを表示する | sidebar itemではowner avatarを先頭へ置き、実Gitのbranchまたはdetached HEAD短縮SHAを最も目立つtitle、GitHub `origin`がある場合はcredentialを除いた`owner/repo`を小さい補助文字で表示する。対象workspaceのmain turnが送信開始、実行中、停止処理中の時はsidebarのowner avatarを同じ24px領域のrunning spinnerへ置き換え、accessible nameとtooltipへlocalized session-running textを追加する。terminal、idle、利用者回答待ちではowner avatarへ戻す。reduced motion時はspinnerの回転だけを停止し、形状、running色、textを残す。headerはGitHub `origin`がある場合にownerのGitHub avatar、`owner/repo`、workspace名をこの順のbreadcrumbとして表示し、branchを隣接表示する。sidebarとheaderのavatarはreferrerを送信せずGitHubの画像originだけから取得し、offline、画像取得失敗、またはGitHub `origin`がないlocal repositoryではapp iconを使わずrepositoryを示すneutral fallbackへ縮退する。GitHub `origin`がないlocal repositoryの文字列は保存済みrepo名へfallbackする。selected itemとheaderの長い値はellipsisと全文tooltipを持つ | Approved | 非該当 |
| `WORK-F-055` | 利用者は最初のprojectを登録してworkspaceを作成できる | registered projectが0件のnative起動では全viewportのoverview setup guideへ未完了の`最初のプロジェクト`だけを表示し、同じnative folder pickerとProject setup dialogから1件目を登録させる。登録完了後はoverviewからこの項目を直ちに除き、他の必須項目も解消済みなら通常shellへ進む。workspaceが0件かつregistered projectが1件以上のmain surfaceには`プロジェクトを選択 / Select a project` titleと、sidebarのWorkspace追加dialogと同じrepository icon画像付き3column project cardを表示する。card選択で既定nameを生成して作成を開始し、workspace name input、select、補助説明、追加のcreate buttonは表示しない。この状態ではsidebarにempty説明文を表示せず、Done / In Review / In Progress / Backlog / Canceledのlifecycle groupを常に表示する | Approved | 非該当 |
...
| `WORK-F-069` | workspace UIの初期描画は空windowへ失敗しない | 0件を含むworkspace selectionを全viewportのsidebarでoptionalとして扱い、compact navigationも存在しないselectionを参照しない。main surfaceまたはdialogのproject cardを選んだ時にlocal timestampとrandom suffixを生成し、WebViewが`crypto.randomUUID`を提供しない場合もfallback suffixを生成して画面を維持する。application moduleの読込またはReact描画が失敗した場合はrootを空のままにせず、localeに合う回復案内と再読込操作を表示する | Approved | 非該当 |

### 継続性と境界

| 要件ID | 要件 | 受け入れ条件 | 状態 | 廃止理由・後継ID |
|---|---|---|---|---|
| `WORK-F-058` | アプリは一度に一つのactive executionだけを許可する | active/pending turnを持つworkspaceから別workspaceを選択しても旧turnをinterruptせず、selectionとviewだけを100ms以内に切り替える。旧workspaceはsidebarでrunningを示し続け、新workspaceでは履歴、Commit、draft、contextを閲覧・編集できるが、旧turnがterminalになるまでSend、attachment登録、decision/approval回答、Stopを旧workspaceと取り違えて実行しない。旧turnがterminalになった後は現在selectionのthread contextを同じApp Server processとconnection上でactivateし、workspace選択または作成をApp Server接続requestの入力や再接続条件にしない。thread contextの一時失敗は対象workspaceのdraftを保持して自動再試行し、接続状態または手動回復操作をworkspace UIへ表示しない。rapid selection、duplicate response、stale terminalでもactive executionを最大1件、誤workspace mutationを0件にする | Approved | 非該当 |
| `WORK-F-059` | workspace切替はview state、execution state、audioを分離する | workspace選択時に旧workspaceのpresentation intentと音声を停止し、新workspace固有のdraftとtimeline anchor ID/sequence/offsetを復元し、anchor消失時だけ最寄りvalid sequenceへ補正する。旧workspaceで継続中のevent/error/pending request/音声を新workspaceのtimeline、Composer、Characterへ表示・再生せず、sidebarのrunning状態だけをworkspace ID一致で示す。commit説明のworkspace切替はpresentation intentだけをrevokeし、app-owned support jobとgenerated/prepared cacheをcancel・削除しない | Approved | 非該当 |
| `WORK-F-060` | アプリはworkspace stateを再起動後に復元する | 20件のworkspaceについてProject ID、group、active selection、draft、last summary、timeline anchor ID/sequence/offsetをnative storeから再起動後に一致させる。app管理worktreeは保存済みmanaged provenanceのworkspace root、`.git` marker、per-worktree Git directory、common Git directoryのcanonical pathとfilesystem identityがlive再導出値へ完全一致した時だけCodex workspaceとしてactivateし、停止中のmarker retargetやGit directory置換を新expectedとして採用しない。managed provenanceのないmigration前rowは履歴を残したままChangedとして明示的にblockし、暗黙の再attestationを行わない。保存anchorが削除・retention・correctionで存在しない場合だけ同workspaceの最寄りvalid sequenceへ補正し、別workspaceのsummary/anchorを再利用しない | Approved | 非該当 |
...
| `WORK-F-063` | 利用者はproject contextとcharacterごとのcharacter contextを分離して編集できる | App settings > Projectsで個別projectを開くと、そのProject IDに属する全workspaceで共有するProject contextを編集できる。App settings > Characterで個別characterを開くと、そのopaque pack IDだけに属するCharacter contextを編集できる。二つは別record、別versionとして保存し、Character contextからtechnical rule、permission、checkpoint policyを変更できない | Approved | 非該当 |
...
app管理linked worktreeのmain Codex write authorityはworkspace rootを既定境界とし、Gitのstage・commitに必要なworkspace固有per-worktree Git directoryと同一repositoryのcommon Git directoryだけを例外として扱う。例外の正本は、app-owned rootでworktreeを作成した同じtransactionが保存するversioned managed provenanceである。provenanceはworkspace root device/inode、`.git` marker device/inode、canonical per-worktree Git directory path/device/inode、canonical common Git directory path/device/inodeを保持する。起動復元と各wire call直前にmarker、Git plumbing、app dataのexact root、保存済み全identityからnativeで再導出・canonicalizeし、current user所有、ancestorを含むnon-group/world-writable、writeable、同一repository identityを全て満たす時だけApp Serverへ渡す。project working tree、他worktreeのper-worktree Git directory、markerから導出されないdirectory、symlinkまたは`..`によるescapeは許可しない。通常のin-tree `.git` repository、手動linked worktree、`--separate-git-dir` repositoryは追加rootを持たせない。migration前のmanaged provenanceがないrowと保存後にoffline retargetされたrowは履歴を残してChangedにし、live値による再attestationを行わない。検証失敗または利用中App Serverの契約で限定追加rootを表現できない場合はwrite authorityを広げずturn開始をfail closedにする。
...
| persistence / migration | `src-tauri/src/workspace_history/store.rs` | `projects.registered`、workspace固有canonical root、`managed_worktree`、migration 8のnullable common Git directory identity、migration 12のmanaged Git provenance、DB version、N-1 fixture |
...
| UI | `src/main.tsx`、`src/app/StartupFailure.tsx`、`WorkspaceProjectSelection.tsx`、`WorkspaceSidebar.tsx`、`SettingsView.tsx`、`WorkspaceShell.tsx` | startup error boundary、zero-workspaceとdialogで共有するproject card選択、hover/focus Archive、compact navigation、Projects登録解除確認 |
...
| Workspace | project | 未選択 | 必須 | 登録済みProject IDをrepository icon画像付きcardで単一選択する。tableとselectは使わない | card一覧を保持し、再選択できるerrorを表示 |
| Workspace | name | card選択時に`ws-MMDD-<random 4文字>`を生成 | 必須 | UIから編集させない。trim後1〜80 Unicode scalar、改行不可。Git branch/worktree pathへはnative側で安全なslugとopaque IDを使用し、表示nameをpathへ直接使用しない | 同じcardから再試行する時に新しい既定nameを生成 |
...
| App settings > Character > character detail | character context | bundled Hiyoriは桃瀬ひより用preset、custom packはpack表示名と中立な既定値 | 任意 | opaque pack ID-scoped。display name 1〜40、tone補足0〜1,000、behavior 0〜4,000、prohibited expressions最大20項目・各1〜200、全field・全項目の総量12,000 Unicode scalar、technical policy key禁止 | 禁止内容を除いて再編集を求める |

### Versioned editable Context contract

`ProjectContext`と`CharacterContext`はcapture済みFiles / Git diffとは別のeditable contextである。ProjectはProject ID、Characterはopaque pack IDをpartition keyとして保存する。Characterの選択自体はapp-globalだが、設定recordと未保存draftはpack間で共有しない。Projectと各Characterは別versionを持ち、片方の保存が他方の未保存draftまたはversionを変更してはならない。
...
| `CharacterContext` | `displayName` | string、trim後1〜40 Unicode scalar。bundled Hiyoriのfresh recordは`桃瀬ひより`、custom packはpack表示名 |
...
Project総量は32,000、Character総量は12,000 Unicode scalarを上限とし、配列の各itemも総量へ加算する。Projectの保存requestは`projectId`と`expectedVersion`、Characterの取得requestは`packId`と初回だけ使うboundedな`displayName`、保存requestは`packId`と`expectedVersion`を必須にする。SQLite transaction内でpartition keyと現在versionの一致を検証してからversionをちょうど1増やし、canonical JSONのSHA-256を更新する。不一致時は`PROJECT-CONTEXT-CONFLICT`または`APP-CHARACTER-CONTEXT-CONFLICT`を返し、DBと利用者のdraftを変更しない。
...
Sendはnative owner-only character libraryから現在選択中のpack IDを解決し、native storeから対象workspaceが属するProject IDのProject contextと選択packのCharacter contextを同じread transactionで取得する。pack ID、各version、canonical JSON hashを持つimmutable request snapshotをturn開始前に一度だけ確定する。snapshot直前に現在のtrusted workspace root、workspace identity、全technical referenceを再検証し、失敗時はrecoverable preflight errorでSendを止める。main sessionへ渡すcontextは命令ではないquoted untrusted dataとして明示的な境界内へ直列化し、authoritative user instructionおよびapp-owned technical policyと混在させない。Character contextはpresentation metadataとしてのみ扱い、permission、verification、安全ruleのauthorityを持たない。turnが`running` / `waiting`になった後の保存やpack選択を途中注入せず、次のSendだけが新versionとpackを取得する。Project record、draft、conflictはProject ID、Character record、draft、conflictはpack IDごとに分離し、turn snapshotだけを対象workspaceへ固定する。

bundled Hiyoriの新規recordは`displayName=桃瀬ひより`、`tone=warm`、`speechDensity=key_events`とし、明るく親しみやすく急かさない話し方、重要な変化と判断時だけ声をかける振る舞い、利用者を責める・過度に馴れ馴れしい・不確かな内容を断定する表現の回避をpresetに持つ。このpresetも利用者が編集でき、保存後は再起動またはpreset更新で上書きしない。schema migration前のapp-global Character contextはHiyoriのpack IDへ一度だけ移す。利用者が編集済みの内容・version・hashは保持し、旧factory defaultの`Sol`と既知hashが完全一致するrecordだけは、versionを維持してHiyori presetとそのhashへ置換する。fresh databaseも同じHiyori presetから始める。
...
| 未保存データ | composer draftはworkspace単位、Project context入力はProject ID単位、Character context入力はpack ID単位で保持 | `WORK-F-059`, `WORK-F-063`, `WORK-F-064` |
...
| `S-006` | ワークスペース設定（廃止） | 非該当 | 廃止 | [画面詳細仕様](../screen-design/S-006_project-settings.md) |
