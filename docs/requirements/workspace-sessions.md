...
| `WORK-F-048` | 利用者は初回起動と送信前にpreflightを確認できる | native workspace読込後、Codex CLIまたはGit executableが利用できない時、選択workspaceのCodex activationが未接続・blocked・failedになった時、またはnative runtimeがerrorになった時は通常shellより先に全viewportのoverview setup guideを表示する。guideはnative readiness snapshotと選択workspaceの接続状態から未解決項目だけを列挙し、ready項目を描画しない。Codexはinstall、login、update、GitはmacOSのinstallを現在のrecovery actionに合わせて日英で示し、commandは利用者が明示的にcopyして外部terminalで実行する。OS、history、Live2D、preferencesの`blocked`または`unavailable`も同じ画面に回復理由を示すが、`warning`だけではoverviewを継続しない。readiness再確認は項目ごとに置かず、未完了一覧全体に対するbuttonを1個だけ表示し、選択workspaceのCodex activationも同じ操作で再試行する。guideから任意shellを起動せず、snapshotまたは接続再確認で解消した項目は直ちに非表示にする。アプリ内の各画面にはCodex、Git、履歴の接続状態をまとめた個別の汎用noticeを表示しない。workspaceではGit、Codex executable、auth、`gpt-5.6-sol`、character packを`ready/warning/blocked`で表示し、blocked項目があればSendを無効にする | Approved | 非該当 |
