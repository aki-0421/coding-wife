...
| `WORK-F-048` | 利用者は初回起動と送信前にpreflightを確認できる | native workspace読込後、Codex CLIのtrusted executableを検出できない、Codex App Serverを起動して`initialize` / `initialized`を完了できない、Git executableが利用できない、native runtimeがerror、registered projectが0件、またはOS、history、Live2D、preferencesが`blocked` / `unavailable`の時だけ、通常shellより先に全viewportのoverview setup guideを表示する。起動時のnative readinessはworkspace復元完了barrierを待ち、復元対象がある場合は保存済みCodex pathを復元済みworkspace rootで検証する。初期readinessが未完了の間はtitle、skeleton、進捗を含む可視UIを表示せず、問題が判明した後だけguideを描画する。guideはnative readiness snapshotから未解決項目だけを列挙し、ready項目を描画しない。Codexのsetup判定ではbinary trust、version、App Server起動と初期化だけを確認し、schema、login、config、model/list、`gpt-5.6-sol`、reasoning effortを待たない。これらの送信前診断は通常workspaceで継続し、失敗時はsetup guideへ戻さずSendだけを無効にする。CodexとGitのinstall commandは利用者が明示的にcopyして外部terminalで実行する。readiness再確認は項目ごとに置かず、未完了一覧全体に対するbuttonを1個だけ表示する。overviewで明示Codex pathのbinary trust、version、App Server初期化と保存に成功した時はguideを直ちに終了し、未接続の選択workspace再接続は追加clickなしで開始するが、その完了をpath fieldまたはguideの待機条件にしない。guideから任意shellを起動せず、snapshot更新で解消した項目は直ちに非表示にする。アプリ内の各画面にはCodex、Git、履歴の接続状態をまとめた個別の汎用noticeを表示しない。workspaceではGit、Codex executable、auth、`gpt-5.6-sol`、character packを`ready/warning/blocked`で表示し、blocked項目があればSendを無効にする | Approved | 非該当 |
