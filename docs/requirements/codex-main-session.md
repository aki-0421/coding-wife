...
|---|---|---|---|---|
| `CODE-F-051` | アプリはCodex App Serverの利用可否を診断し、単一processを所有する | executableは明示設定、GUI processの`PATH`、時間・出力上限を持つ利用者のdefault login shell、既知の安全なinstall位置の順で探索し、候補のcanonical path、所有者、書込権限、実行権限、version、hashをRustで検証する。default login shellは実行ファイルの探索だけに使用し、検証後のApp Serverはexact canonical binaryを直接起動してCodex管理のloginを利用する。startup restore後にtrusted active workspaceとbinaryが利用可能ならApp Serverを1件だけspawnしてinitialize、schema、protocol capability、login、config、model/listを確認し、アプリ終了まで同じprocessを全workspaceで共有する。workspace選択、thread開始・再開、readiness再確認では追加App Serverをspawnせず、同じconnection上のworkspace/thread contextだけを切り替える。binary identity変更、process crash、protocol violationだけは旧process treeの停止完了後にreplacementを1件起動でき、同時にliveなmain App Serverを2件にしない。失敗段階の安全なcodeと回復操作をSend可否へ反映し、通常workspace側の失敗だけでは初回setupへ戻さない。利用者は初回setupとGeneral settingsでCodex executableの絶対pathを任意指定または自動検出へ戻すことができ、初期化に成功したcanonical pathだけをapp-private設定へ保存する | Approved | 非該当 |
...
| `CODE-F-076` | workspace閲覧切替時に旧turnを混在させない | turn開始と全mutationをactivation token、workspace、thread、App Server generationへ束縛する。view selectionはApp Server connection ownershipと独立させ、旧workspaceのactive turnを背景で継続する。選択後に到着した旧workspace event/error/pending requestは旧timelineへだけ保存し、新workspace timeline、Composer、Live2D stateへ表示しない。stale `turn/start`がacceptedならexact旧turnをinterruptし、そのterminalだけを旧workspaceへ保存する | Approved | 非該当 |
