...
|---|---|---|---|---|
| `CODE-F-051` | アプリはCodex App Serverの利用可否を診断し、単一processを所有する | executableは明示設定、GUI processの`PATH`、時間・出力上限を持つ利用者のdefault login shell、既知の安全なinstall位置の順で探索し、候補のcanonical path、所有者、書込権限、実行権限、version、hashをRustで検証する。default login shellは実行ファイルの探索だけに使用し、検証後のApp Serverはexact canonical binaryを直接起動してCodex管理のloginを利用する。startup restore後にworkspace非依存のapp-private runtime rootからApp Serverを1件だけspawnしてinitialize、schema、protocol capability、login、config、model/listを確認し、アプリ終了まで同じprocessとconnectionを全workspaceで共有する。App Serverの接続requestとdiagnosticにworkspace IDを含めず、workspace選択、作成、thread開始・再開、repository recheckでは接続判定、再接続、追加spawn、再initializeを行わない。同じconnection上でworkspace/thread contextだけを切り替える。binary identity変更、process crash、protocol violationでは旧process treeの停止完了後にreplacementを1件だけ自動起動し、上限付き指数backoffでhandshake成功まで再試行する。handshake成功後は選択中workspaceの所有threadを追加操作なしで開始または再開し、同時にliveなmain App Serverを2件にしない。接続状態、接続error code、接続badge/banner、Reconnect/Retry操作はheader、Composer、Character、workspace固有UIへ表示せず、復旧中はtimeline、navigation、workspace固有draftを維持してSendだけを内部状態から安全側で無効にする。特定workspaceの通常thread server errorまたはstale handleでApp Serverを再起動せず、他workspaceのSend可否を変更しない。利用者は初回setupとGeneral settingsでCodex executableの絶対pathを任意指定または自動検出へ戻すことができ、初期化に成功したcanonical pathだけをapp-private設定へ保存する | Approved | 非該当 |
...
| `CODE-F-054` | appはactive workspaceのcwdと最小write authorityでmain threadを開始する | canonical workspace rootとselected effortを使ってthreadを1件開始し、別workspace pathを使用しない。`workspace-write`の既定write rootはworkspace rootだけとする。app管理linked worktreeでは、turn直前のnative preflightがworkspaceのnon-symlink `.git` markerからcanonicalizeし、保存済みper-worktree Git directory identityと同一repositoryのGit common directory identity、current user ownership、ancestorを含むnon-group/world-writable policy、writeabilityを再検証したGit metadata directoryだけを追加write rootにする。project working tree全体、他worktreeのper-worktree Git directory、任意pathを追加せず、identity不一致、symlink・escape、owner・mode・writeability不一致、App Serverが追加rootを表現できない場合はturnを開始しない。通常のin-tree `.git` repositoryでは追加rootを送らない | Approved | 非該当 |
...
| `CODE-F-074` | process crash後にturnを自動再送しない | child終了時にturnをInterruptedとし、draftと受信済みeventを維持する。App Server再起動と選択中workspace threadの再開はアプリが自動実行するが、受理済みまたは送信中だった利用者turnは自動再送せず、復旧後の新しいturn開始を待つ | Approved | 非該当 |
...
| `CODE-F-076` | workspace閲覧切替時に旧turnを混在させない | turn開始と全mutationをactivation token、workspace、thread、App Server generationへ束縛する。view selectionとworkspace thread contextはApp Server connection ownershipと独立させ、旧workspaceのactive turnを背景で継続する。選択後に到着した旧workspace event/error/pending requestは旧timelineへだけ保存し、新workspace timeline、Composer、Live2D stateへ表示しない。新workspaceのthread開始・再開が一時失敗した時は対象workspaceのdraftを保持してアプリが自動再試行し、App Server接続error、raw code、手動Retryを対象workspaceまたは他workspaceへ表示しない。自動再試行後も解消しないworkspace固有の非接続failureだけは通常のsession errorとして対象workspaceへ表示できる。stale `turn/start`がacceptedならexact旧turnをinterruptし、そのterminalだけを旧workspaceへ保存する | Approved | 非該当 |
...
| App Server復旧中 | timeline閲覧可、draft保持、Send無効。接続状態と手動回復操作は表示せず自動復旧 | `CODE-F-051`, `CODE-F-074`, `CODE-F-075` |
