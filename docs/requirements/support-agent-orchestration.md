...
| `SUP-F-078` | app controllerは説明状態とUI intentを一意に調停する | `CommitExplanationControllerStateV1.status`を`not_generated` / `queued` / `running` / `generated` / `failed` / `unavailable` / `canceled`に限定する。新しいverified commitはbackgroundで`queued`へ自動遷移するがpresentation eventを作らない。`user_request` / `user_retry`の1回の操作は、未生成、同じkeyのqueued/running、generated cache hitのいずれでもexact request/selectionへrebindしたpresent-on-complete intentになる。selection / locale / workspace / Stop / Close後はintent epochを失効させ、late event/responseを表示しない。Stop / Closeはsupport job/cacheを維持し、生成Cancelだけがsupport taskをterminal化する | Approved | 非該当 |
