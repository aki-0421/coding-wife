...
description: "主セッションと分離した短命支援root、redacted context、構造化出力、停止・監査を定義する。"
updated: 2026-07-18
read_when:
...
状況要約、判断説明、review補助をmain coding threadへ混在させると人格と履歴が不明瞭になる。一方、支援agentへrepositoryやsecretを渡すと権限と費用が拡大するため、短命・分離・失敗非伝播の調停が必要である。
...
| 対象 | 内容 |
|---|---|
| Roles | presence/narration、decision explainer、checkpoint reviewerをdeterministic triggerで起動 |
| Isolation | mainとは別ephemeral root、toolなし、redacted snapshot、固定diff例外 |
| Output | role別versioned JSON schema、stale detection、fallback |
...
| 通常支援のrepo/shell access | least privilegeを守る | fixed review diffだけ例外 |
...
| ファイル・OS操作 | 通常supportは非該当。reviewerもfixed payloadだけ | `SUP-F-053`, `SUP-F-054` |
...
| 領域 | 要件 |
|---|---|
| セキュリティ | least privilege、tool/cwdなし、fixed diff例外、prompt injectionを命令として実行しない |
| 権限 | support outputは技術policy、DOM、filesystem、Live2D pathを操作できない |
| プライバシー | secret/raw reasoning/source本文を通常snapshotへ含めず、raw session historyを保存しない |
...
| アクセシビリティ | support状態はtextで示し、Live2D/音声だけに依存しない |
...
| 論点 | 初期判断 | 確認事項 | 着手ブロック |
|---|---|---|---|
| role初期構成 | presence/narrationをP0、decision/checkpointをP1としてschemaを先に定義 | vertical slice後に実測token/latencyで順序を確認する | いいえ |
