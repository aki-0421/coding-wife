...
| 秘密保護 | secret値、生reasoning、コード本文、音声dataがSQLite、index、backupへ入らない。 |
...
|---|---|---|---|---|
| HIST-F-001 | app-owned履歴の正本をOS application data内の単一SQLite databaseにする。 | process再起動後も保存済みrecord数と非秘密fieldが一致し、履歴をnetworkまたはWeb Storageへ複製しない。 | Draft | 非該当 |
...
| HIST-F-020 | terminal記録のない実行を自動再実行しない。 | crash後に`running`/`waiting_for_user`のturn・command・assignmentを`予期しない中断`または`結果不明`eventで閉じ、prompt、command、Git、testを自動送信しない。 | Draft | 非該当 |
| HIST-F-021 | 1 eventと対応evidenceをatomic transactionで書く。 | event insert、typed evidence、参照、session last sequenceの途中にfaultを注入しても全件commitまたは全件rollbackとなり、孤立参照と半端なeventが0件である。 | Draft | 非該当 |
| HIST-F-022 | 履歴write失敗を成功扱いにしない。 | I/O、disk full、lock失敗は1回だけ自動retryし、再失敗時に`HIST_WRITE_FAILED`と手動retryを表示する。成功済みtransactionだけを再表示し、重複eventを作らない。 | Draft | 非該当 |
...
| HIST-F-024 | migrationまたはintegrity check失敗時に元databaseを保護する。 | transactionをrollbackし元databaseとbackupを上書きせず、sidecar/sessionを開始しない。S-004にpathを伏せたerror code、backup有無、再試行、新規database開始を表示する。 | Draft | 非該当 |
...
| HIST-F-026 | SQLite writerを1 process・1 queueへ限定する。 | 二重起動、main/support同時event、削除、migration試験でwriterが最大1つ、write transactionがFIFO直列となり、2個目のprocessはread/write connectionを開かない。 | Draft | 非該当 |
...
| question | request/question ID、非secret質問・回答、secret回答済みflag、resolved UTC | secret値なし |
...
| integrity/migration失敗 | `HIST_DB_INTEGRITY_FAILED`または`HIST_MIGRATION_FAILED` | 元DBとbackupを上書きしない |
...
| 閉じる・アプリ終了 | closeではwriteと実行を継続し、Quitでは共通transaction後にcloseする。 | HIST-F-019〜HIST-F-022 |
...
| プライバシー | HIST-F-007、HIST-F-013〜HIST-F-018をprimary、journal、index、backup、cache、logへ同一適用する。 |
...
| 信頼性・復旧 | WALとdurable transactionを使用し、fault injectionでHIST-F-020〜HIST-F-025を検証する。完了不明の外部操作を成功へ補正しない。 |
...
| [codex-main-session要件](../codex-main-session/requirements.md) | main thread/turn、AskUserQuestion、secret分類を提供する | Draft | HIST-F-004、HIST-F-006、HIST-F-007を検証できない |
