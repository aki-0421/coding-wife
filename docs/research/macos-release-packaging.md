...
3. `scripts/release/build-macos-app.sh`は毎回`/private/tmp`のmode `0700` private targetとmode `0600` logからbuildする。private `umask`を継承したTauri outputのapp rootだけはseal前に配布mode `0755`へ正規化する。production native codeはbundled Hiyoriをinstall済みresource directoryだけから解決し、`CARGO_MANIFEST_DIR`によるsource-tree fallbackは`cfg(test)`へ限定する。raw stdout/stderrをconsoleへ返さず、dependency licenseの非stale判定、repositoryとpackaged legal treeのbyte一致、nested-first seal、full product verifyを通したcandidateだけを指定outputまたはstandalone canonical pathへpublishする。combined commandでは指定したprivate outputだけを作り、canonical pathへ直接書かない。
...
12. raw command stderr、input/output/tempのabsolute pathはconsoleへ返さない。失敗はstep単位のsafe codeと非0 exitで表し、app verification失敗だけはverifier自身のallowlist済み`APP_*` codeをinner causeとして伝播する。trapはmountを先に確実にdetachし、今回runのprivate workを安全なprefix確認後に削除し、combined backup/journalを復元してからlockを解放する。publish commitはcanonical verifier成功後にjournal/backupを削除した時点だけとする。
