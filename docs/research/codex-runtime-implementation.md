...
11. native RUIが使えない場合のassistant最終完了文は`result`または`decision_request`のJSON全体だけを受理する。Codexへ渡す`outputSchema`はStructured Outputsが受理する単一root objectとし、全fieldをrequiredにしたうえで、backendが受理するbounds、fixed value、array count、nested exact objectをRust parserと同じにする。variant固有fieldはbackendが受理するschema keywordでnull/non-null関係を表現できる場合だけschemaにも固定し、表現できない関係をdescriptionだけで保証済みと扱わない。`result`では`decisionId`、`question`、`options`、`context`をnullに限定し、booleanまたはnullの`allowFreeform`はdecisionへ影響しない互換fieldとして無視する。`decision_request`では必要な値がすべてnon-nullかつ`allowFreeform=false`であることをRustでも再検証する。option横断unique、recommendation membership、privacyはparserを最終正本とする。Structured Outputのassistant deltaはJSON envelopeのtransport断片なのでWebView eventへ出さない。完了itemはApp Serverの`phase`を確認し、`commentary`ならredact・長さ制限した途中経過、`final_answer`またはphaseなしなら厳格検証した最終出力として扱う。commentaryがexact `result` envelopeの場合は`message`だけを取り出し、commentaryのplain textまたはJSON envelopeを最終回答違反としてinterruptしない。最終回答の自由文、freeform decision、approval代替、不正optionは受理せずactive turnをinterruptする。
