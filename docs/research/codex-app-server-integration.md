...
## ephemeral support agent の gate

| 必要条件 | 0.144.x の今回の証拠 | 判定 |
| --- | --- | --- |
| ephemeral=true | response で true | 対応 |
| thread/list に残らない | 同じ接続で非列挙 | 対応。ただし全永続先の不変は別監査が必要 |
| cwd なし | response は必須 AbsolutePathBuf、実測も workspace cwd | 非対応 |
| runtime workspace root なし | 実測 1 件 | 非対応 |
| dynamic tool 0 | dynamicTools=[] は client-defined tool だけ | 未証明 |
| shell / file / MCP 0 | thread 単位の完全 allowlist が schema に無い | 未証明 |
| raw prompt / response 非永続 | アプリ側では可能 | 対応可能 |

よって CapabilitySnapshotV1.supportIsolation=unavailable とし、通常 support session、固定 reviewer support session とも 0 にする。代替は次の決定的処理だけで構成する。

- main event から状態、失敗、待機、完了をルールで集約。
- Git diff、test result、checkpoint metadata をローカル parser で要約。
- 既知 error code から固定 guidance を選択。
- AI の提案や自動判断を生成しない。

将来の再評価では、明示的な built-in tool allowlist / deny-all、null cwd、null workspace root が protocol に追加され、生成 schema と runtime probe の双方で確認できることを最低条件にする。モデルへ「使える tool を答えて」と尋ねる自己申告は隔離証明にしない。

## test fixture 計画

fixture は個人情報、実 path、prompt、account を含まない。各 fixture に sourceCliVersion、schemaFingerprint、fixtureSchemaVersion を付ける。

推奨 fixture:

- initialize-ok.jsonl
- initialize-not-initialized.jsonl
- initialize-already-initialized.jsonl
- initialize-experimental-rejected.jsonl
- model-list-sol-low-max.jsonl
- model-list-sol-missing.jsonl
- model-list-max-missing.jsonl
- thread-start-ok.jsonl
- thread-resume-ok.jsonl
- thread-list-paginated.jsonl
- thread-start-model-mismatch.jsonl
- turn-stream-complete.jsonl
- turn-stream-interrupted.jsonl
- turn-stream-disconnect-before-terminal.jsonl
- approval-command.jsonl
- approval-file-change.jsonl
- approval-permissions.jsonl
- approval-unknown-method.jsonl
- request-user-input-valid.jsonl
- request-user-input-invalid.jsonl
- dynamic-tool-valid.jsonl
- dynamic-tool-unknown.jsonl
- review-detached.jsonl
- event-unknown-notification.jsonl
- event-unknown-item.jsonl
- transport-malformed.jsonl
- transport-oversized.jsonl
- ephemeral-support-isolation-failed.jsonl

Rust tests:

- JSONL framing、partial read、複数行、UTF-8、上限。
- request id correlation、duplicate、timeout、server request response。
- initialize state machine と experimental fallback。
- model / effort gate と model reroute。
- start / resume / list pagination。
- interrupt が turn/completed まで terminal でないこと。
- 3 種 approval の allowlist と unknown fail-closed。
- decision schema の exact validation。
- dynamic tool registry の schema validation。
- normalization、redaction、unknown quarantine。
- child crash、shutdown 5 秒、restart budget。

TypeScript tests:

- IPC response の exact key validation。
- DomainEvent v1 の kind / producer / payload validation。
- workspace generation が変わった event の破棄。
- approval / decision UI が unsupported payload で表示されないこと。
- raw path、stderr、reasoning が state へ入らないこと。

opt-in live test:

- 選択したローカル CLI から schema を生成し、accepted fingerprint と比較。
- initialize、account/read、model/list。
- ephemeral thread/start と thread/list 非列挙。
- 実際に token を消費する turn / review test は CODEX_LIVE_E2E=1 の明示時だけ。
...
src-tauri/src/codex/schema_probe.rs
src-tauri/src/codex/process.rs
src-tauri/src/codex/jsonl.rs
src-tauri/src/codex/rpc.rs
src-tauri/src/codex/protocol.rs
src-tauri/src/codex/capabilities.rs
src-tauri/src/codex/session.rs
src-tauri/src/codex/approval.rs
src-tauri/src/codex/decision.rs
src-tauri/src/codex/dynamic_tools.rs
src-tauri/src/codex/review.rs
src-tauri/src/codex/normalizer.rs
src-tauri/src/codex/redaction.rs
src-tauri/src/codex/diagnostics.rs
src-tauri/src/codex/support.rs
src-tauri/src/codex/commands.rs
src-tauri/tests/codex_contract.rs
src-tauri/tests/codex_supervisor.rs
~~~

protocol.rs は使用する subset DTO だけを手書きで定義し、source CLI version と schema fingerprint を定数で関連付ける。生成物全体を application runtime API として公開しない。schema_probe.rs は同一 binary の生成物から method / field の存在と fingerprint だけを抽出する。

TypeScript:

~~~text
src/lib/contracts/codex.ts
src/lib/contracts/ipc.ts
src/lib/contracts/domain-event.ts
src/features/codex/CodexProvider.tsx
src/features/codex/transport.ts
src/features/codex/session-store.ts
src/features/codex/approval-store.ts
src/features/codex/decision-store.ts
src/features/codex/diagnostics.ts
src/test/fixtures/codex-app-server/v1/*.jsonl
~~~

既存の ipcSchemaVersion と domainEventSchemaVersion は wire adapter version と分ける。OpenAI schema が変わっても WebView 契約を同時に破壊しないためである。

## 実装順

1. fixture、Rust subset DTO、CompatibilitySnapshotV1 を先に固定する。
2. binary discovery、schema probe、process supervisor、JSONL transport を実装する。
3. initialize、account/read、model/list と diagnostics を実装する。
4. thread start / resume / list、turn start / interrupt を実装する。
5. event normalizer、redaction、DomainEvent / IPC を接続する。
6. 3 種 approval と requestUserInput fail-closed を実装する。
7. detached review を実装する。
8. 具体的な製品 tool が承認された場合だけ dynamic tool registry へ追加する。
9. support agent は隔離 gate が通るまで実装せず、決定的 fallback を維持する。

各段階で local fake server fixture test を先に通し、最後に opt-in live probe を行う。CLI 更新時は schema fingerprint 差分をレビューし、unknown を無理に既知型へ丸めない。
