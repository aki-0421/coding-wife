...
3. **GPT-5.6 Terra (`gpt-5.6-terra`)** is an isolated, zero-tool commit explainer. Once a reachable commit is verified, the native controller starts Terra silently in the background with bounded read-only Git evidence. **Explain changes** presents the cached explanation or reuses that same running job; it does not initiate the first Terra handoff or join the write-capable main session.
...
The three roles have different authority and latency needs. Sol needs the complete coding context and tools. Luna needs a fast, privacy-bounded signal for human presence, but no repository tools. Terra needs only verified immutable Git evidence: the native controller starts generation automatically and silently, while the user controls only presentation. Keeping those roles isolated makes the experience more expressive without giving presentation features coding authority or mixing support output into the main work history.
...
6. When the commit becomes reachable and verified, confirm the app starts Terra silently in the background, then open **Commit changes** and inspect a changed file in the unified diff.
7. Select **Explain changes** and confirm it presents a cached explanation or waits for the already-running Terra job. The action must not initiate the first Terra handoff.

Expected result: Sol completes real repository work, Luna keeps the character visibly responsive without gaining tools, verified commit evidence starts one silent Terra background job, the Git tab shows the reviewable patch, and **Explain changes** presents or reuses that job's result.
...
| 2:00–2:18 | Note that verified commit evidence already started Terra silently, then select **Explain changes** and show the cached or running result. | “When this commit became reachable and verified, the native controller started Terra silently in the background. Explain changes presents that cache, or waits for the same running job; it never starts the first handoff or grants write access.” |
...
- State that verified reachable commit evidence starts Terra silently in the background. Use **Explain changes** only to present cached output or reuse the running job; do not describe it as the first Terra trigger.
...
| Commit explainer | `gpt-5.6-terra` | Native auto-dispatch of bounded read-only evidence after a reachable commit is verified | Silent cached explanation; explicit presentation can reuse the running job | Zero tools; no repository authority |
...
| Verified commit evidence starts Terra silently, while explicit action only presents or reuses it | `src-tauri/src/codex/commit_explanation.rs`; `src-tauri/src/codex/support_isolation.rs`; Git review explanation adapter | Auto-generation, silent cache, running-job reuse, presentation, and isolation tests; video 2:00–2:18 |
...
- [ ] Confirm the macOS release shows the same Sol/Luna flow, silent automatic Terra generation, explicit explanation presentation, and Git review behavior claimed in the video.
