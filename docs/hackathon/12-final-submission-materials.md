...
| Supported platforms | macOS 14+ Apple Silicon; Windows 11 x64; Ubuntu 22.04 / Debian 12-compatible Linux x64 |
| Current release URL and SHA-256 | `PENDING` |
...
- **Hosted demo:** Not applicable unless one is actually published. The desktop test build is the no-rebuild judge path.
...
3. **GPT-5.6 Terra (`gpt-5.6-terra`)** is an isolated, zero-tool commit explainer. When the developer selects **Explain changes**, Terra turns bounded read-only Git evidence into a concise explanation without joining the write-capable main session.
...
The three roles have different authority and latency needs. Sol needs the complete coding context and tools. Luna needs a fast, privacy-bounded signal for human presence, but no repository tools. Terra needs only immutable Git evidence and a user-triggered explanation request. Keeping those roles isolated makes the experience more expressive without giving presentation features coding authority or mixing support output into the main work history.
...
The current production path requires a compatible authenticated local Codex installation and a writable Git repository. Packaging targets macOS 14+ Apple Silicon, Windows 11 x64, and Ubuntu 22.04 / Debian 12-compatible Linux x64. Full end-to-end product QA is currently centered on macOS; Windows and Linux have package installation or extraction verification rather than equivalent workflow E2E. Optional OpenAI TTS requires a user-supplied API key and playback is currently macOS-only; captions remain the cross-platform fallback. The app does not provide cloud sync or remote collaboration. The frozen release notes must state the exact signing and notarization status.

## 3. Judge testing instructions

### Recommended no-rebuild path

1. Download the **current submission release** from the URL entered in Devpost and verify its published SHA-256.
2. Follow the platform-specific installation note attached to that release.
...
6. When the session completes, open **Commit changes**. Select the commit and a changed file, then inspect the unified diff.
7. Select **Explain changes** and confirm that Terra provides a concise explanation of the bounded commit evidence.

Expected result: Sol completes real repository work, Luna keeps the character visibly responsive without gaining tools, the Git tab shows the reviewable patch, and Terra explains the selected commit on request.
...
Use this path to inspect the implementation or run the current checkout:
...
The final video must remain under 3:00 and contain clear English audio. Record the current native build on a disposable repository. Configure optional TTS before recording and never show its key. Cut loading and typing pauses rather than speeding up the explanatory moments.
...
| 2:00–2:18 | Select **Explain changes** and show Terra's concise result. | “When I choose Explain changes, GPT-5.6 Terra receives bounded read-only Git evidence and explains the patch. Terra is isolated from the write-capable session and cannot change the repository.” |
...
| 2:36–2:50 | Show a short commit/test montage, then the closing card with repository and platform. | “We used Codex to implement, debug, test, and QA this pipeline. Humans chose the trust boundaries and review experience. The repository, release, and testing instructions are included in the submission.” |
...
- Use **Explain changes** for Terra. Do not describe Sol as the commit explainer.
...
| Commit explainer | `gpt-5.6-terra` | User-triggered bounded read-only commit evidence | Concise explanation of the selected changes | Zero tools; no repository authority |
...
| Terra explains without write authority | `src-tauri/src/codex/commit_explanation.rs`; `src-tauri/src/codex/support_isolation.rs`; Git review explanation adapter | Isolation and explanation tests; video 2:00–2:18 |
...
| Current public release and artifact SHA-256 | `PENDING` |
...
- [ ] Download the current submission release without authentication, verify its SHA-256, install it, and complete the primary flow without rebuilding.
- [ ] Confirm the release shows the same three-model and Git review behavior claimed in the video.
